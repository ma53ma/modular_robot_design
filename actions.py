...
def masking(probs,mod_size, prev_action):
    ...
    #probs = [i / norm for i in probs]
    #sum = np.sum(probs)
    #probs = [i / sum for i in probs]

    return probs

def choose_action(network,a, mod_size, prev_action, goal, ep):
...
# reward function
def reward(a, curr, mod_size, goal, action, modules, env):
    ...
    if action == mod_size - 1 or curr == len(a) - mod_size: # if putting end effector on or at last module, obtain terminal reward
        dist, term_r, end_eff_pos = term_reward(a, mod_size, goal, curr, modules, env)
        rew += term_r
        pos = end_eff_pos
    return rew, dist, pos

def term_reward(a, mod_size, goal, curr, modules, env):
    arrangement = [''] * int((curr / mod_size) + 1)
    info = ['DQN-Result', '0.0.1']
    for i in range(int(curr / mod_size) + 1):
